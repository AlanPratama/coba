...
@section('content')
    <div class="flex justify-start items-center gap-3">
        <div>
            <div x-data="{ tambahPeminjaman: false }" x-on:keydown.window.escape="tambahPeminjaman = false">
                <div class="">
                    <button type="button" x-on:click="tambahPeminjaman = !tambahPeminjaman"
                        class="flex justify-center items-center gap-2 rounded border border-primary-600 mb-3 bg-primary-600 px-3 py-1 text-center text-sm font-medium text-white shadow-sm transition-all hover:border-primary-700 hover:bg-primary-700 focus:ring focus:ring-primary-200 disabled:cursor-not-allowed disabled:border-primary-300 disabled:bg-primary-300"><i
                            class="fa-solid fa-plus font-bold text-2xl"></i> <span>TAMBAH ENTRI PEMINJAMAN</span></button>
                </div>
                <div x-cloak x-show="tambahPeminjaman" x-transition.opacity class="fixed inset-0 z-10 bg-secondary-700/50">
                </div>
                <div x-cloak x-show="tambahPeminjaman" x-transition
                    class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
                    <form action="{{ route('peminjaman.store') }}" method="POST"
                        class="mx-auto overflow-hidden rounded-lg bg-white shadow-xl sm:w-full sm:max-w-xl">
                        @csrf
                        <div class="relative p-6">
                            <button type="button" x-on:click="tambahPeminjaman = false"
                            ...
                            <h3 class="text-lg font-medium text-secondary-900">TAMBAH ENTRI PEMINJAMAN</h3>
                            <div class="mt-2 text-sm text-secondary-500 grid grid-cols-1 gap-5">
                                <div>
                                    <label for="f_status" class="mb-1 block text-sm font-medium text-gray-700">Status
                                        Peminjaman</label>
                                    <input type="text" name="f_status" id="f_status" value="Dipinjam" readonly disabled
                                        required
                                ...
                                <div>
                                    ...
                                    <input type="text" name="f_idadmin" id="f_idadmin" required
                                        value="{{ Auth::guard('admin')->user()->f_nama }}" disabled
                                        class="block w-full rounded-md border-gray-300 text-gray-800 shadow-sm border py-2 px-3 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" />
                                </div>
                                <div>
                                    <label for="f_tanggalpeminjaman"
                                        class="mb-1 block text-sm font-medium text-gray-700">Tanggal Peminjaman</label>
                                    <input type="date" name="f_tanggalpeminjaman" id="f_tanggalpeminjaman" required
                                        class="block w-full rounded-md border-gray-300 text-gray-800 shadow-sm border py-2 px-3 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" />
                                </div>
                                <div>
                                    <label for="f_idanggota"
                                ...
                                <div>
                                    ...
                                    <select name="f_iddetailbuku" id="f_iddetailbuku"
                                        ...
                                        @foreach ($buku as $bk)
                                            <option value="{{ $bk->f_id }}">{{ $bk->buku->f_judul }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
        ...
        <div class="flex justify-start items-center">
            <div x-data="{
                ...
                <div x-ref="panel" x-show="open" x-transition.origin.top.left x-on:click.outside="close($refs.button)"
                    ...
                    <div class="p-1">
                        <form action="{{ route('peminjaman.streamPdf') }}" method="POST">
                            @csrf
                            <button type="submit"
                        ...
                        <form action="{{ route('peminjaman.downloadPdf') }}" method="POST">
                            @csrf
                            <button type="submit"
                            class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-gray-700 hover:bg-gray-100">
                        ...
                        <td class="pl-2 pr-3 py-4">
                            <div class="flex justify-end gap-4">
                                ...
                                <div>
                                    <div x-data="{ editPeminjaman{{ $dp->f_id }}: false }"
                                        ...
                                        <div x-cloak x-show="editPeminjaman{{ $dp->f_id }}" x-transition
                                            class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
                                            <form
                                                action="{{ route('peminjaman.update', ['id' => $dp->peminjaman->f_id]) }}"
                                                method="POST"
                                                class="mx-auto overflow-hidden rounded-lg bg-white shadow-xl sm:w-full sm:max-w-xl">
                                                @method('PUT')
                                                @csrf
                                                    ...
                                                    <div class="mt-2 text-sm text-secondary-500 grid grid-cols-1 gap-5">
                                                        <div>
                                                            ...
                                                            <input type="text" name="f_status" id="f_status"
                                                                value="Dipinjam" readonly disabled required
                                                                class="block w-full rounded-md border-gray-300 text-gray-800 shadow-sm border py-2 px-3 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" />
                                                        </div>
                                                        <div>
                                ...
                                <div>
                                    ...
                                    <div>
                                        <div x-data="{ dikembalikan{{ $dp->f_id }}: false }"
                                            x-on:keydown.window.escape="dikembalikan{{ $dp->f_id }} = false">
                                            <div class="flex justify-center">
                                                <button type="button"
                                                    x-on:click="dikembalikan{{ $dp->f_id }} = !dikembalikan{{ $dp->f_id }}">
                                ...
                                S1.293,9.212,1.729,9.212z"></path>
                                            ...
                                            <div x-cloak x-show="dikembalikan{{ $dp->f_id }}" x-transition.opacity
                                                class="fixed inset-0 z-10 bg-secondary-700/50"></div>
                                            <div x-cloak x-show="dikembalikan{{ $dp->f_id }}" x-transition
                                                class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
                                                <div
                                                    class="mx-auto overflow-hidden rounded-lg bg-white shadow-xl sm:w-full sm:max-w-xl">
                                                    <div class="relative p-6">
                                                        <div class="flex gap-4">
                                                            ...
                                                            <div class="flex-1">
                                                                <h3 class="text-lg font-medium text-secondary-900">Apakah
                                                                    Peminjam Sudah Mengembalikan Buku?</h3>
                                                                <div class="mt-2 text-sm text-secondary-500">Status
                                                                    Peminjaman Akan Berubah Menjadi <b>Dikembalikan</b>
                                                                    Ketika Tombol <b>Confirm</b> Diklik</div>
                                                            </div>
                                                        </div>
                                                        <form
                                                            action="{{ route('peminjaman.dikembalikan', ['id' => $dp->f_id]) }}"
                                                            method="POST" class="mt-6 flex justify-end gap-3">
                                                            @method('put')
                                                            @csrf
                                                            <button type="button"
                                                                x-on:click="dikembalikan{{ $dp->f_id }} = false"
                                                                class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-center text-sm font-medium text-gray-700 shadow-sm transition-all hover:bg-gray-100 focus:ring focus:ring-gray-100 disabled:cursor-not-allowed disabled:border-gray-100 disabled:bg-gray-50 disabled:text-gray-400">Cancel</button>
                                                            <button type="submit"
                                                                class="rounded-lg border border-primary-500 bg-primary-500 px-4 py-2 text-center text-sm font-medium text-white shadow-sm transition-all hover:border-primary-700 hover:bg-primary-700 focus:ring focus:ring-primary-200 disabled:cursor-not-allowed disabled:border-primary-300 disabled:bg-primary-300">Confirm</button>
                                                        </form>
